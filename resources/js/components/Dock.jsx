...
const ICONS = {
    home: (props) => (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" {...props}>
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0 7-7 7 7m-2-2v10a1 1 0 0 1-1 1h-3m-6 0a1 1 0 0 0 1-1v-4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 0 1 1m-6 0h6" />
        </svg>
    ),
    user: (props) => (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" {...props}>
    ...
    kelas: (props) => (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" {...props}>
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m12 14 9-5-9-5-9 5 9 5zm0 0 6.16-3.422A12.083 12.083 0 0 1 18.825 17.057 11.952 11.952 0 0 0 12 20.055a11.952 11.952 0 0 0-6.824-2.998 12.078 12.078 0 0 1 .665-6.479L12 14Zm-4 6v-7.5l4-2.222" />
        </svg>
    ),
    calendar: (props) => (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" {...props}>
...
function DockIcon({ name, active, disabled }) {
    ...
    const colorClass = disabled ? 'text-slate-300' : active ? 'text-red-600' : 'text-slate-600';
...
function DockItem({ item, mouseX, spring, distance, magnification, baseItemSize }) {
    ...
    const baseClasses = 'relative inline-flex items-center justify-center rounded-full border-2 shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white';
...
export default function Dock({
    ...
    const maxHeight = useMemo(() => Math.max(dockHeight, magnification + magnification / 2 + 8), [dockHeight, magnification]);
    const heightRow = useTransform(hoverState, [0, 1], [panelHeight, maxHeight]);
    const height = useSpring(heightRow, spring);

    const sanitizedItems = Array.isArray(items) ? items : [];

    return (
        <motion.div style={{ height }} className={`flex w-full items-end justify-center ${className}`}>
            <motion.div
                onMouseMove={({ pageX }) => {
                    hoverState.set(1);
                    mouseX.set(pageX);
                ...
                className="flex w-fit items-end gap-3 rounded-3xl border-2 border-slate-200 bg-white/95 px-4 pb-3 shadow-lg"
                role="toolbar"
