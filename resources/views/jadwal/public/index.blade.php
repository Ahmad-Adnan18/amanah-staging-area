            ...
            <div x-show="selectedItem" class="mt-6 sm:mt-8 bg-white rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6" x-cloak>
                ...
                <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
                    ...
                    <div class="w-full sm:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                        <a :href="activeTab === 'guru-libur' ? '{{ route('jadwal.public.print_all_guru_libur') }}' : '{{ url('/jadwal/print-all') }}/' + activeTab + '/semua_jadwal.pdf'" target="_blank" class="inline-flex items-center gap-2 justify-center rounded-md bg-slate-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-slate-600">
            ...
            Alpine.data('scheduleViewer', () => ({
                ...
                get printUrl() {
                    ...
                    if (this.activeTab === 'guru-libur') {
                        ...
                        return `{{ url('/jadwal/print/guru-libur') }}/${this.selectedDay}/guru_libur.pdf`;
                    }

                    if (this.activeTab === 'pelajaran') {
                        // [NEW] URL untuk print pelajaran
                        return `{{ url('/jadwal/print/pelajaran') }}/${this.selectedSubject}/jadwal_mapel.pdf`;
                    }

                    const type = this.activeTab === 'kelas' ? 'kelas' : 'guru';
                    const id = this.selectedItem;
                    return `{{ url('/jadwal/print') }}/${type}/${id}/jadwal.pdf`;
                },
