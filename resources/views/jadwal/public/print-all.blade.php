...
<body>
    ...
    @foreach($items as $item)
        @php
            ...
            $teachingHoursSummary = $item['teachingHoursSummary'] ?? [];
                            ...
                            @foreach($timeSlots as $timeSlot)
                                @if(isset($schedules[$dayKey][$timeSlot]))
                                    @foreach($schedules[$dayKey][$timeSlot] as $schedule)
                                        {{-- In logic printAll, schedules are already filtered for the subject, so we dont need if subject check unless we want to sure --}}
                                        <tr>
                                ...
                                @foreach($timeSlots as $timeSlot)
                                    <table class="slot-table">
                                        <tr>
                                            ...
                                            @if(isset($schedules[$dayKey][$timeSlot]))
                                                {{-- Handle potential array of schedules if structure is consistent, though for class/teacher usually 1 unless collision --}}
                                                @php 
                                                    $schedule = is_array($schedules[$dayKey][$timeSlot]) ? $schedules[$dayKey][$timeSlot][0] : $schedules[$dayKey][$timeSlot]; 
                                                @endphp
