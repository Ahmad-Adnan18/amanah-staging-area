...
<aside class="fixed inset-y-0 left-0 z-[70] flex w-64 transform flex-col bg-white text-gray-900 border-r border-slate-200 transition-all duration-300 ease-in-out -translate-x-full md:translate-x-0 shadow-xl" :class="{ 'translate-x-0': sidebarOpen }" x-cloak>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6" x-data="{
        ...
        <!-- GRUP: NOTIFIKASI -->
        @if(in_array(Auth::user()->role, ['admin', 'pengajaran', 'pengasuhan', 'kesehatan', 'ubudiyah']))
        <div class="pt-4 mt-4 border-t border-slate-200">
            <a href="{{ route('admin.notifications.index') }}" class="{{ $baseClasses }} {{ request()->routeIs('admin.notifications.*') ? $activeClasses : $inactiveClasses }}">
                <div class="flex items-center justify-center w-5 h-5">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                    </svg>
                </div>
                <span>{{ __('Kelola Notifikasi') }}</span>
            </a>
        </div>
        @endif
    </nav>
