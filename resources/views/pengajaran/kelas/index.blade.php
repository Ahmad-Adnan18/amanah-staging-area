<x-app-layout>
    {{-- [TAMBAHAN] Alpine.js dibutuhkan untuk menu aksi 'kebab' dan pop-up --}}
    @push('scripts')
            ...
            <div class="space-y-6 sm:space-y-8" x-data="{
                ...
                <div x-show="showDeleteConfirm" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
                    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 max-w-sm w-full mx-4">
                        ...
                        <p class="mt-2 text-sm text-slate-600">Apakah Anda yakin ingin menghapus kelas ini? Semua santri di kelas ini juga akan terhapus. Tindakan ini tidak dapat dibatalkan.</p>
                ...
                <!-- Pop-up Konfirmasi Generate -->
                <div x-show="showGenerateConfirm" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
                    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 max-w-sm w-full mx-4">
                        <h3 class="text-lg font-semibold text-slate-900">Konfirmasi Generate Kode</h3>
                        <p class="mt-2 text-sm text-slate-600">Apakah Anda yakin ingin membuat kode untuk semua santri yang belum memilikinya? Proses ini tidak dapat dibatalkan.</p>
                        <div class="mt-6 flex justify-end gap-3">
                            <button @click="cancelAction()" class="inline-flex items-center px-4 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-md hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2">
                                Batal
                            </button>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6">
                    ...
                    <div class="space-y-4">
                        <form action="{{ route('pengajaran.kelas.index') }}" method="GET">
                            <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
                                <div class="flex-grow relative">
                                    ...
                                    <input type="text" name="search" id="search" value="{{ request('search') }}" class="block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm sm:text-base" placeholder="Cari santri berdasarkan nama atau NIS...">
                                </div>
                        ...
                        <div class="border-t border-slate-200 pt-4 flex flex-col sm:flex-row items-center gap-3">
                            <p class="text-sm text-slate-600 flex-shrink-0">Aksi Generate Kode:</p>
                            <div class="flex flex-col sm:flex-row items-center space-x-0 sm:space-x-2 space-y-2 sm:space-y-0 w-full sm:w-auto">
                                <form action="{{ route('pengajaran.kelas.generate_all_wali_codes') }}" method="POST" x-ref="generateWaliCodesForm">
                                    @csrf
                                    <button type="button" @click="confirmGenerate($refs.generateWaliCodesForm)" class="w-full sm:w-auto inline-flex items-center gap-2 justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                                ...
                                <a href="{{ route('pengajaran.kelas.export_wali_codes') }}" class="w-full sm:w-auto inline-flex items-center gap-2 justify-center rounded-md bg-green-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200">
                    <div class="p-4 border-b border-slate-200">
                        <h3 class="font-semibold text-gray-800">Hasil Pencarian untuk "{{ request('search') }}"</h3>
                    </div>
                    <div class="divide-y divide-slate-200">
                        @foreach ($hasilPencarianSantri as $santri)
                        <div class="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center hover:bg-slate-50">
                            <div>
                                <p class="font-bold text-slate-800 text-sm sm:text-base">{{ $santri->nama }}</p>
                                <p class="text-xs sm:text-sm text-slate-500">NIS: {{ $santri->nis ?? '-' }} | Kelas: {{ $santri->kelas->nama_kelas ?? 'N/A' }}</p>
                            </div>
                            <a href="{{ route('santri.profil.show', $santri) }}" class="mt-2 sm:mt-0 text-xs sm:text-sm font-medium text-red-600 hover:text-red-800">Lihat Profil &rarr;</a>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="border-l-4 border-yellow-400 bg-yellow-50 p-4 rounded-r-lg">
                    <p class="text-sm text-yellow-700">Santri dengan nama atau NIS "{{ request('search') }}" tidak ditemukan.</p>
                </div>
                @endif
                @endif
                ...
                <div>
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-4">
                        ...
                        @can('create', App\Models\Kelas::class)
                        <a href="{{ route('pengajaran.kelas.create') }}" class="w-full sm:w-auto mt-2 sm:mt-0 inline-flex items-center justify-center gap-2 rounded-md bg-red-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-600">
                            ...
                            <div class="p-4 flex justify-between items-start">
                                ...
                                <div x-data="{ open: false }" class="relative flex-shrink-0 -mr-2 -mt-2">
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl ring-1 ring-black ring-opacity-5 py-1 z-10" x-cloak>
                                        ...
                                        @endcan
                                        @can('delete', $kelas)
                                        <form action="{{ route('pengajaran.kelas.destroy', $kelas) }}" method="POST" x-ref="deleteForm-{{ $kelas->id }}">
                                            @csrf @method('DELETE')
                                            <button type="button" @click="confirmDelete($refs['deleteForm-{{ $kelas->id }}'])" class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50">Hapus Kelas</button>
                                        </form>
                                        @endcan
                                    </div>
                                </div>
                    ...
                    <div class="hidden md:block bg-white rounded-2xl shadow-lg border border-slate-200 overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="px-4 sm:px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Nama Kelas</th>
                                    <th class="px-4 sm:px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Penanggung Jawab</th>
                                    <th class="px-4 sm:px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Ruangan Induk</th>
                                    <th class="px-4 sm:px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Status Jadwal</th>
                                    <th class="px-4 sm:px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Jumlah Santri</th>
                                    <th class="px-4 sm:px-6 py-3.5 text-right text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-slate-200">
                                @forelse ($kelas_list as $kelas)
                                <tr class="hover:bg-slate-50">
                                    <td class="px-4 sm:px-6 py-4 font-medium text-slate-900 align-top whitespace-nowrap">
                                        {{ $kelas->nama_kelas }}
                                        <div class="text-xs text-slate-500 font-normal">Tingkatan: {{ $kelas->tingkatan ?? 'N/A' }}</div>
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 text-slate-500 align-top">
                                    ...
                                    <td class="px-4 sm:px-6 py-4 text-slate-500 align-top whitespace-nowrap">{{ $kelas->room->name ?? 'N/A' }}</td>
                                    ...
                                    <td class="px-4 sm:px-6 py-4 text-slate-500 align-top whitespace-nowrap">{{ $kelas->santris_count }} santri</td>
                                    <td class="px-4 sm:px-6 py-4 text-right space-x-4 align-top">
                                        <a href="{{ route('pengajaran.santris.index', $kelas) }}" class="font-medium text-red-600 hover:text-red-800 text-xs sm:text-sm">Lihat Santri</a>
                                        @can('update', $kelas)
                                        <a href="{{ route('pengajaran.kelas.edit', $kelas) }}" class="font-medium text-slate-600 hover:text-slate-900 text-xs sm:text-sm">Edit</a>
                                        @endcan
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="6" class="px-4 sm:px-6 py-12 text-center text-slate-500">Belum ada data kelas.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
