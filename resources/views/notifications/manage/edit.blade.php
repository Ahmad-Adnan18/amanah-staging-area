    ...
    <div class="bg-slate-50 min-h-screen">
        <div class="max-w-3xl mx-auto py-6 sm:py-8 px-4 sm:px-6 lg:px-8 pb-24 sm:pb-8">
            ...
            <div class="mb-6 sm:mb-8">
                ...
                <p class="mt-2 text-slate-600 text-sm sm:text-base">Ubah data notifikasi di bawah</p>
            </div>

            {{-- Form --}}
            <form action="{{ route('admin.notifications.update', $notification) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-5">
                    ...
                    <textarea name="message" id="message" rows="5" required
                        ...
                        placeholder="Masukkan isi pesan notifikasi">{{ old('message', $notification->message) }}</textarea>
                    @error('message')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select name="type" id="type" required
                        ...
                        @foreach($types as $value => $label)
                            <option value="{{ $value }}" {{ old('type', $notification->type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                ...
                <div class="mb-5">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        @php
                            $selectedRoles = old('target_roles', $notification->target_roles ?? []);
                        @endphp
                        ...
                        <label class="flex items-center gap-2 p-3 border border-slate-200 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors">
                            <input type="checkbox" name="target_roles[]" value="{{ $value }}" 
                                ...
                                {{ is_array($selectedRoles) && in_array($value, $selectedRoles) ? 'checked' : '' }}>
                ...
                <div class="mb-5 p-4 bg-slate-50 rounded-xl">
                    ...
                    <div id="schedule-fields" class="mt-4 space-y-4">
                        <div>
                            <label for="published_at" class="block text-sm font-medium text-gray-700 mb-2">Jadwal Publish</label>
                            <input type="datetime-local" name="published_at" id="published_at" 
                                value="{{ old('published_at', $notification->published_at?->format('Y-m-d\TH:i')) }}"
                                class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-5 p-4 bg-green-50 border border-green-200 rounded-xl">
                    <div class="flex items-center gap-2 text-green-700">
                        ...
                        <span class="font-medium">Sudah dipublish</span>
                    </div>
                    <p class="text-sm text-green-600 mt-1">{{ $notification->published_at->format('d M Y, H:i') }}</p>
                </div>
                @endif
